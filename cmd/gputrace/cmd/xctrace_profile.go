...
type xctraceProfileTOCSummary struct {
	XML        string                `json:"xml"`
	Export     privateReplayerResult `json:"export"`
...
var xctraceProfileCmd = &cobra.Command{
	...
	Short:        "Export a headless xctrace profile into streamData",
	SilenceUsage: true,
	Long: `Export headless xctrace Metal tables and encode target-attributed
Metal GPU intervals into .gpuprofiler_raw/streamData.

This command is fail-closed for counters: it reports counter table row counts
but only allows timing claims from encoded streamData.`,
	Args: cobra.NoArgs,
...
func init() {
	...
	xctraceProfileCmd.Flags().IntVar(&xctraceProfileMaxRows, "max-rows", 20000, "Maximum interval rows to encode")
...
func runXctraceProfile(cmd *cobra.Command, args []string) error {
	...
	if !out.TimingClaimsAllowed {
		return fmt.Errorf("xctrace profile did not produce usable timing streamData")
	}
	return nil
}
...
func runXctraceProfileExport() (*xctraceProfileOutput, error) {
	...
	out := &xctraceProfileOutput{
		...
		Reason:               "timing claims require exported xctrace Metal GPU interval rows encoded as streamData; hardware counter claims remain disabled until non-empty counter samples are parsed",
	}
	...
	out.TimingClaimsAllowed = streamData.TimingUsable && streamData.StreamData.Present && streamData.RowsEncoded > 0
	out.CounterClaimsAllowed = false
	out.Reason = "timing claims use xctrace Metal GPU intervals encoded as streamData; hardware counter claims remain disabled until non-empty counter samples are parsed"
	return out, nil
}
...
func runXctraceProfileCommand(name string, argv []string, outputDir string) privateReplayerResult {
