...
This command does not synthesize timing. It reports whether the bundle contains
.gpuprofiler_raw/streamData, which is required for real profiler timing and
counter-derived claims.`,
...
func inspectProfilePayload(tracePath string) (*profileInspectOutput, error) {
	...
	if out.GPUProfilerRaw.Present && out.StreamData.Present {
		if stats, err := counter.ParseStreamData(profilerDir); err == nil {
			...
			if stats.DerivedCounterSampleCount() > 0 {
				out.ProfilerCounters = true
			}
		}
	}
	if out.RealTiming && len(out.CountersFiles) > 0 {
		out.ProfilerCounters = true
	}
