...
intervals, and encode target-attributed rows into .gpuprofiler_raw/streamData.
...
func init() {
	...
	headlessProfileCmd.Flags().IntVar(&headlessProfileMaxRows, "max-rows", 20000, "Maximum interval rows to encode")
...
func runHeadlessProfile(cmd *cobra.Command, args []string) error {
	...
	if !profile.TimingClaimsAllowed {
		return emitHeadlessProfileOutput(out, fmt.Errorf("headless profile did not produce usable timing streamData"))
	}
