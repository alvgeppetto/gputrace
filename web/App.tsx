...
import { transformTraceToGraph } from './utils/graph';
...
export default function App() {
  ...
  useEffect(() => {
    fetchTraceData()
      .then(data => {
        // Backend returns single root, Sidebar expects array currently
        setTraceData([data]);
        setLoading(false);
      })
      .catch(err => {
  ...
  const graphData = useMemo(() => {
    if (!traceData.length) return { nodes: [], edges: [] };
    return transformTraceToGraph(traceData[0]);
  }, [traceData]);
  ...
  const handleSelect = (id: string | null) => {
    ...
    let item = findTraceItem(traceData, id);
